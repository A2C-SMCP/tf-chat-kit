...
import { expectedPackedManifest } from "./packed-artifact-policy.mjs";
import { PACKAGE_POLICY } from "./workspace-policy.mjs";

const rootDirectory = process.cwd();
const outputDirectory = path.join(rootDirectory, ".artifacts", "packages");
const stagingDirectory = path.join(rootDirectory, ".artifacts", "pack-staging");
...
try {
  for (const directory of Object.keys(PACKAGE_POLICY)) {
    ...
    const stagedPackageDirectory = path.join(stagingDirectory, directory);
    await cp(packageDirectory, stagedPackageDirectory, {
      recursive: true,
      filter: (source) =>
        !path
    ...
    await cp(
      ...
      path.join(stagedPackageDirectory, "LICENSE"),
    );
    const sourceManifest = JSON.parse(
      await readFile(path.join(packageDirectory, "package.json"), "utf8"),
    );
    const publishManifest = expectedPackedManifest(sourceManifest);
    await writeFile(
    ...
    if (typeof packOutput.filename !== "string") {
      throw new Error(
        `${expectedName}: pnpm pack did not return a tarball path`,
      );
    }
