...
**核心原则**：需求先于代码，公共性先于实现，复用优于新建，方案以最佳实践根治根因，实现后必须经隔离上下文客观复审。

> **可选尾参** `--review block|all|discuss`（默认 `block`）：控制 Phase 5.5 的修复范围，透传给 `turingfocus-toolkit:fix-review`。
...
- **形态 A—灵感/模糊想法**：进入 Interview 循环。每轮先探索代码与文档，再带着证据说明洞察，最后提不超过 3 个会改变方案的问题。
- **形态 B—完整方案/详细需求**：解析现有决策与验收标准，不重复访谈已明确内容。
- **关联工作项**：Phase 0 只读取 Jira/GitHub/CNB Issue 及其 parent/Epic，提取整体目标与约束作为后续“北极星”；状态统一在 Phase 0.5 推进。方案与父级目标冲突时，在 Phase 2 显式标注并商议。
...
- 输入含 Jira/GitHub/CNB Issue 时核对其层级与变更规模；层级不足时只补建必要父级并链接已有项，不复制描述。
- 未提供 Issue 但仓库已配置追踪系统且工具可用时，按规模建立最小结构。
- 仓库尚未配置追踪系统或工具不可用时，不得假造编号；记录缺口并与用户对齐是否继续。追踪缺口不得被描述为已完成。
- 父级只承载北极星和聚合进度，不为同一变更复制描述。
- **状态推进是硬门槛**：已关联或本阶段新建工作项时，只推进当前开始实施的最底层项到“进行中”；未推进时禁止进入 Phase 1。Jira 使用 transition，GitHub/CNB 添加 `in-progress` 标记。

**准出**：追踪结构与用户对齐；已有/新建工作项已推进，或工具缺口已显式记录且用户同意继续。
...
6. 将 Story、Issue 和文档中的路径、文件、命名与当前代码逐条核对；它们是写作时快照，不是强制落点。偏差在 Phase 2 显式列出实际落地与理由。
7. 在改动被引用符号前验证 caller、公共导出、动态/反射入口、构建条件和跨仓消费者，防止把功能建在已被取代的死路径上。确认为死代码且妨碍当前任务时，先提供证据和外科手术式清理范围，获用户确认后再删除。
...
- 新建且属于本 Feature 的代码/测试文件形成可审查的最小完整内容后，先用 `git status --short` 确认其为 `??`，再立即执行 `git add -- <精确文件路径>`，使其进入 `git diff --cached` 供隔离审查。只暂存本任务新文件；禁止 `git add .`、`git add -A`、暂存目录、已有修改文件或无关文件。
...
测试通过不等于可交付。进入 Phase 6 前必须完成以下闭环：

1. **组装审查输入包**：由当前代理显式整理，不依赖父会话继承。
   - **需求原文**：传入用户原话；来自 Issue/链接时，先由当前代理读取，再传“来源 + 标题 + 完整需求正文 + 验收标准”；来自本地文档/附件时，传文档标识与相关完整内容。禁止只传链接、Issue Key 或文件路径。
   - **已确认需求**：传入 Phase 0–2 确认的用户场景、范围结论、Kit/宿主责任、验收标准、非目标、权限/数据约束和已批准决策；与原文不同时显式列出用户确认的变更。
   - **审查范围**：传入仓库路径、基线、本 Feature 文件清单、测试/门禁结果。启动前执行 `git status --short`，本 Feature 新代码/测试文件不得仍为 `??`；如有，按 Phase 4 逐个精确暂存。
2. **严格隔离复审**：启动只读 reviewer（优先 `explorer`），显式设置 `fork_turns="none"`。Prompt 只包含上述输入包、加载 `turingfocus-toolkit:code-review` rubric 的要求和审查输出格式；禁止传实施对话、实现者推理、自评或倾向性修复建议。要求 reviewer 自行执行 `git status --short`、`git diff` 和 `git diff --cached`，结合完整文件审查全部变更，不暂存、不修改任何文件。
3. **验真并修复**：将 reviewer 报告作为来源交给 `turingfocus-toolkit:fix-review`；`block`（默认）只处理 ❌，`all` 处理 ❌+⚠️，`discuss` 逐条讨论。先验证问题真实性，不盲修误报；任何 ❌ 都必须有结论。
4. **重新隔离复审**：修复后启动新的 `fork_turns="none"` 只读 reviewer，传入更新后的同结构输入包。可附上历史问题作待验证项，但必须重新审查完整 diff。存在 ❌ 则继续循环，直到阻塞项清零。

若当前环境不能启动独立 reviewer，明确标记门禁未完成并请求用户决定，不得把同一实现者的自查伪装成隔离审查。

## Phase 6：交付

> **交付边界**：Phase 4 为保证新文件进入隔离审查而执行的 `git add -- <精确新文件路径>` 是唯一预授权例外。未获用户对本次变更的明确授权前，不暂存已有修改或无关文件，不执行 commit、push、发布或创建 PR。
...
获得提交授权后，再把可用的 commit/分支信息回写追踪系统。自底向上更新追踪结构：当前项完成后附交付摘要；父 Story/Epic 仅在所有子项完成时关闭，否则只追加进度。若 Jira workflow 要求 commit hash/修复分支，在状态迁移前先回写真实值；不猜测字段 ID，从同项目已完成项的 schema 反查。

## 关键决策点

需求与范围结论（Phase 0）｜追踪方案与状态推进（Phase 0.5）｜技术方案（Phase 2）｜外部依赖（Phase 3）｜实施计划（Phase 4）必须与用户对齐。用户明确表示“直接做”或信任代理判断时可连续执行，但不跳过步骤、ADR 或硬门槛。
...
| 新代码文件保持 `??`，导致 reviewer 只看到已跟踪 diff | 形成可审查内容后按精确路径暂存，同时审查 `git diff` 与 `git diff --cached` |
| reviewer 继承实施会话，或只收到 Issue/文档路径 | 使用 `fork_turns="none"`，由当前代理传入完整审查输入包 |
| 把 Story/文档快照当成当前代码真相 | 核对实际路径、caller 和公共消费者，偏差在方案中显式说明 |
