...
流程对齐 `turingfocus-toolkit:add-feature`（本次基线：0.5.1）；本文件保留可独立执行的流程，以本仓库章程、Accepted ADR 和范围参考替代通用版的 SaaS 定位与其他项目资源。

**核心原则**：需求先于代码，公共性先于实现，复用优于新建，技术建议须独立评审，方案以最佳实践根治根因，实现后必须经隔离上下文客观复审。
...
9. **轮询必须审批**：优先事件驱动。确需轮询（包括定时拉取、sleep-loop 探测状态）时，先说明替代方案为何不可行、间隔/退避/上限/资源成本，并提供可核验的业界同类案例，获得用户批准后实施。
...
- **形态 B—完整方案/详细需求**：解析现有决策与验收标准，不重复访谈已明确内容；范围判定后仍须进入 Phase 0.5。需求附带的技术建议先标记为待评审，不自动视为已批准方案。
...
- 数据/服务端契约、认证材料来源、适用角色/权限和 capability 差异。
...
沿用通用 `add-feature` 的最小科学追踪原则：跨模块/项目、多轮协作的大型需求使用 Epic→Story→Issue，单模块独立交付的中型需求使用 Story，可一次收敛的小型需求使用单 Issue；GitHub/CNB 按父 Issue→子 Issue 表达相应层级。复用已有工作项，不重复创建，也不因需求较小而降级已有层级。

- 输入含 Jira/GitHub/CNB Issue 时核对其层级与变更规模；层级不足时只补建必要父级并链接已有项，不复制描述。
- 未提供 Issue 时，先读取项目追踪配置；未显式声明时核对 `package.json` 的 `bugs` 与 `git remote -v`，本仓库当前指向 GitHub Issues，不直接套用通用版的 Jira 默认值。平台确定且工具可用时，按规模建立最小结构，小型功能也至少有一个记录。
...
**入口门槛**：确认已满足 Phase 0.5 准出条件；关联项尚未推进时先返回处理。
...
无外部依赖时跳过。存在依赖时，为每个外部项目形成独立请求，包含请求方、关联功能、业务背景、用非技术语言描述的期望能力、优先级（P0 阻塞/P1 重要/P2 一般）、非约束性建议、契约/配置/鉴权/错误/兼容交付要求。对外发送请求遵守当前会话授权。

- 服务端字段、事件或认证语义变化由 TFRobotServer 负责人确认；Kit 不替服务端拍板。
- 宿主身份、路由、布局或平台 API 由 Host Integrator 实现；Kit 只提供必要的稳定端口。
- 阻塞项等待外部交付，非阻塞部分可按已批准边界继续并标明 TODO；收到回复文档后对比原建议与实际契约，调整方案。不得用临时跨层耦合绕过阻塞。

## Phase 4：审批计划与实施

先给出按依赖自底向上的计划：Protocol → Gateway/Runtime → React → UI → Testing/Consumer validation。列出每个文件的改动、测试和迁移步骤；用户审批后再编码。

**测试形态触发器**：先按下列条件判断，再选择 Phase 5 的分层测试；不能仅凭目标包决定测试形态。

1. **打通、改道或启用执行路径**（如发送/中断命令、认证恢复、capability/Feature Flag 启用入口）：测试须从真实调用入口端到端驱动受影响路径，证明用户期望结果成功，不能只断言分支、配置或 mock 调用。
2. **触及真实设施契约**（如 REST 路由、Socket.IO 协议、认证握手、服务端 DTO 或跨 ID 空间映射）：须运行承载所改契约的真实设施，禁止 mock 掉该层。内存 Gateway 可验证 Runtime 语义，但不能替代真实 Gateway/服务端契约验证。

命中任一触发器时，在计划中列出对应新增测试、所需设施与环境、成功断言及执行命令；这些测试必须实际运行至 PASS，作为 Phase 5 的交付前置条件。
...
- 计划外独立阻塞问题建立工作项并关联 Block 关系，暂停受阻部分；当前上下文小问题先说明并获确认。不得顺手修改宿主代码或扩张 V1。
...
**实际执行门禁**：命中 Phase 4 任一触发器时，为该功能新增的真实设施/端到端测试须在本次任务中实际跑至 PASS。只提供命令、收集测试（`--collect-only`）、跳过用例或 mock 掉所改契约层均不算通过。设施或凭证不可用、执行权限不足时，给出准确命令与环境前提，交用户实际运行并取得结果；在结果通过前标记交付门禁未完成，不得仅列风险后宣称功能完成。

项目明确标为 user-trigger 的全量集成/重型 E2E 套件仍交用户触发；上述新增定向测试是交付前置，不能据此免跑。测试要求不授予发布、生产变更或访问凭证的权限。若本仓库或受影响消费者已有 UAT/Seed 体系，交付前评估场景与种子数据影响；宿主侧修改仍遵守跨仓授权边界。
...
- 测试命令、通过结果、Phase 4 触发器判定与对应真实执行证据、未运行项和真实环境风险；适用时附 UAT/Seed 影响。
...
| 打通路径或修改真实契约后，只跑 mock 单测或列出待运行命令 | 对应新增测试端到端驱动真实路径/设施并实际 PASS，未通过前不算完成 |
