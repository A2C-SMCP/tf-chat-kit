...
> 后续增强：GitHub #23 在不改变本基线选择、响应式和宿主所有权结论的前提下，增加了公共可调整
> 双栏比例 API 与无障碍分隔条。跨刷新持久化仍由宿主负责；仓库 Playground 只作为宿主实现示例。
...
| 桌面主区域为左侧时间线、分隔条、右侧详情；没有选择时右侧显示空态。                                | `ChatPlayerContent.tsx:692-746`；`ChatPlayerContainer.css:34-49, 156-224`                                    | 宽容器的 `auto` 模式解析为默认 56%/44% 双栏；未选择时显示安全空态。TFCK-17 首版不提供拖拽，后续 #23 增加 20%–80% 的公共可调整比例。                | UI composition             | 容器测量、双栏比例、空态、拖拽与模式切换测试       |
...
| Front 用可拖拽宽度、Context 中的 current event/play mode 和全局 Socket 数据驱动播放器。           | `ChatPlayerContent.tsx:47-78, 520-559, 749-799`                                                              | 不迁移 Context、全局 Socket 或隐式存储。#23 以后只重建可拖拽的稳定 UI 语义：比例按组件实例隔离并支持宿主受控持久化，不读取 Gateway 或宿主 Store。  | UI composition             | 多实例、会话切换、卸载、拖拽与消费者测试           |
